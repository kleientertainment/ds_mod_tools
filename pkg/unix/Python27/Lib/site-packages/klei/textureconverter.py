...
def Convert( src_filenames, dest_filename, texture_format="bc3", no_premultiply=False, force=False, platform='opengl',
    ...
    if isinstance( src_filenames, list ):
        src_filenames = src_filenames[0]

    if isinstance( src_filenames, list ):
        raise NotImplementedError('Support for multiple input files has not been implemented.')
        src_filename_str = ';'.join( src_filenames )
        for filename in src_filenames:
            is_newer = is_newer or util.IsFileNewer( filename, dest_filename )
    else:
    ...
    if force or is_newer:
        ...
        if no_premultiply:
            raise NotImplementedError('Support for not premultiplying alpha has not been implemented.')
            cmd_list.append( '--no-premultiply' )

        if width:
            raise NotImplementedError('Support for constrained width has not been implemented.')
            cmd_list.append( '-w {}'.format( width ) )
        if height:
            raise NotImplementedError('Support for constrained height has not been implemented.')
            cmd_list.append( '-h {}'.format( height ) )
