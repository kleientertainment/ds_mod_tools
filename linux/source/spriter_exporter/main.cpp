...
{
    return anim.length > 0 && anim.timelines.size() > 0;
}
    ...
    )
{
    ...
    angle = object.angle;
	alpha = object.a;
    if(object.found_scale_x)
    {
        scale.x = object.scale_x;
    }
    if(object.found_scale_y)
    {
                ...
                {					
					...
					if(is_valid_timeline(timeline))
					{
						...
						for(s_timeline_key_map::iterator key_iter = timeline.keys.begin(); key_iter != timeline.keys.end(); ++key_iter)
						{
							...
							strcpy(timeline_key_names[timeline_key_index], scml.folders.find(key.object.folder)->second->name.c_str());
	...
	for(int anim_index = 0; anim_index < anim_count; ++anim_index)
	{
		...
		for(int frame_num = 0; frame_num < anim_length; frame_num+=1000/FRAME_RATE)
		{
			...
			for(int bone_index = 0; bone_index < bone_count; ++bone_index)
			{
				...
				if(fb.parent_id >= 0)
				{
					int parent_id = fb.parent_id;
					for(int parent_bone_index = 0; parent_bone_index < bone_count; ++parent_bone_index)
					{
						if(bone_frames[parent_bone_index].id == parent_id)
						{
							bone& parent = flattened_bone_frames[parent_bone_index];
							fb.scale_x*=parent.scale_x;
							fb.scale_y*=parent.scale_y;

							float sx = fb.x * parent.scale_x;
							float sy = fb.y * parent.scale_y;
							float s = sin(parent.angle);
							float c = cos(parent.angle);
							fb.x= parent.x + sx*c - sy*s;
							fb.y= parent.y + sx*s + sy*c;
							fb.flattened = parent.flattened * fb.flattened;
							fb.angle+=parent.angle;
							fb.parent_id = -1;
							break;							
						}
					}
					if(parent_id == fb.parent_id)
					{						
						error("ERROR: Could not find parent id!");
					}
				}
						...
						for(int bone_index = 0; bone_index < bone_count; ++bone_index)
						{
							...
							if(b.id == parents[element_index])
							{
								...
								create_trans_rot_scale_pivot_matrix(
									IN  float2(b.x, -b.y),
									IN  b.angle,
									IN  float2(b.scale_x, b.scale_y),
									IN  float2(0, 0),
									OUT b.flattened 
									);
								matrices[element_index] =  b.flattened * matrices[element_index];
								*/
							}
						}
					}
